...
<div class="h-full w-full bg-white p-8">
  <div class="mx-auto h-full w-[60%] p-4 flex flex-col gap-12">
      <h1 class="text-5xl font-black">{{ $blog->title }}</h1>
        <div class="w-full flex items-center gap-2">
          <div class="w-12 h-12 rounded-full bg-center bg-cover bg-[url({{ asset('storage/' . $blog->user->image_url)}})]"></div>
          <div>
            ...
            <p class="text-gray-400 text-sm flex items-center gap-2">{{ $blog->reading_time }} min read <span class="text-xs">•</span> <span class="text-md text-gray-600">{{\Carbon\Carbon::parse($blog->created_at)->format('Y-m-d')}}</span></p>
          </div>
        </div>
        <div class="border-y-2 border-gray-100 text-sm flex justify-around items-center gap-6 p-2 py-4">
          ...
          <div>
            <div class="h-[25rem] w-full bg-[url({{ asset('storage/' . $blog->image)}})] bg-cover bg-center"></div>
          ...
          <div class="bg-gray-200 shadow-lg p-4 space-y-6">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-4">
                <div class="w-16 h-16 rounded-lg bg-cover bg-center bg-[url({{ asset('storage/' . $blog->user->image_url)}})]"></div>
                <h1 class="text-4xl font-black">{{$blog->user->name}}</h1>
              </div>
          ...
          <div class="flex flex-col gap-8">
            ...
            <form action="{{ route('comment.store') }}" method="post" class="flex gap-4 items-center mb-4">
                @csrf
                <div class="h-12 w-12 bg-cover bg-center rounded-full bg-[url({{ auth()->user() && auth()->user()->image_url ? asset('storage/' . auth()->user()->image_url) : 'https://images.unsplash.com/photo-1654010816950-55f59acb385a?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D' }})]" for=""></div>
                ...
                @if($blog->comments->first())
                    @foreach($blog->comments as $comment)
                        <div class="flex gap-4 items-start">
                            <div class="h-12 w-12 bg-cover bg-center rounded-full bg-[url({{ $comment->user->image_url ? asset('storage/' . $comment->user->image_url) : 'https://images.unsplash.com/photo-1654010816950-55f59acb385a?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D' }})]" for=""></div>
                            <div class="flex flex-col w-full gap-2">
                                <p class="flex items-center gap-2">{{ $comment->user->name }} <span class="text-xs text-gray-400">•</span> <span class="text-md text-gray-600">{{ \Carbon\Carbon::parse($comment->created_at)->format('Y-m-d') }}</span></p>

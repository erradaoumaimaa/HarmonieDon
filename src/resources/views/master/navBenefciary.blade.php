...
<li class="relative">
    ...
    <div id='notifs-menu' class="absolute top-[calc(100% + 10px)] rounded p-2 w-64 bg-[#d97654] shadow-md hidden">
        @if(auth()->user())
            @foreach(auth()->user()->notifications()->limit(5)->get() as $n)
                <p class="w-full text-sm text-black py-1 px-2">{{ $n->data['donors'] . ' has applied for ' . $n->data['donation']}}</p>
            @endforeach
        @endif
    </div>
</li>
  ...
  <div class="relative ml-4">
    ...
    <li class="flex items-center p-1 font-sans text-sm antialiased font-semibold leading-normal gap-x-2 {{ request()->routeIs('users.profile') ? 'text-[#E44A19]' : 'text-white' }} cursor-pointer active">
  <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg" class="{{ request()->routeIs('users.profile') ? 'text-[#E44A19]' : 'text-white' }}">
  ...
  <a href="{{ route('users.profile') }}" class="flex items-center {{ request()->routeIs('users.profile') ? 'text-[#E44A19]' : 'text-white' }}">
    Profile
  </a>
</li>
